...
import { readableStreamToString, BrexTeamError, parseIfJson } from "./error";
        ...
        if (e instanceof AxiosError && e.isAxiosError) {
            try {
                const responseBody =
                    ...
                    : e.response?.data
                throw new BrexTeamError(e, parseIfJson(responseBody))
            } catch (innerError) {
                if (innerError instanceof ReferenceError) {
                    ...
                    throw new BrexTeamError(e, e.response?.data)
                }
                if (innerError instanceof BrexTeamError) {
                    // Got "BrexTeamError" from the above try block
                    throw innerError;
                }
